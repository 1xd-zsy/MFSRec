...
class JointEmbederBoost(nn.Module):
    def __init__(self, config,args):
        super(JointEmbederBoost, self).__init__()
    ...
    def forward(self, mashup_desc, mashup_desc_len, mashup_index , api_index ):
        ...
        #print("Lstm_repr",Lstm_repr.shape)
        #print("Ncf_repr", Ncf_repr.shape)
        outputs = []
        outputs.append(Lstm_repr)
        outputs.append(Ncf_repr)
        outputs = torch.stack(outputs, dim=-1)
        weighted_outputs = torch.matmul(outputs, self.weights)

        #repr = torch.max(Lstm_repr,Ncf_repr)
        return self.logistic(weighted_outputs)
