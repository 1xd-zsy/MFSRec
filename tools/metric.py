...
def metric3(batch_target, batch_pred, top_k):
    batch_size = batch_target.size(0)
    _ndcg = 0
    _recall = 0
    _map = 0
    _pre = 0
    #print("metric:",batch_target.shape,batch_pred.shape)#metric: torch.Size([128, 1647]) torch.Size([128, 1647])
    for _target, _pred in zip(batch_target, batch_pred):
        ...
        _ndcg += ndcg(_target, _pred[:top_k])
        _recall += recall(_target, _pred[:top_k])
        _map += ap(_target, _pred[:top_k])
        _pre += precision(_target, _pred[:top_k])
